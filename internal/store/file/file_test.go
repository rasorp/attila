...
func Test_New(t *testing.T) {
	...
	t.Run("existing state", func(t *testing.T) {
		...
		createResp, err := fileStore.Region().Create(&store.RegionCreateReq{Region: mockRegion})
		must.Nil(t, err)
		...
		getResp, err := fileStore.Region().Get(&store.RegionGetReq{RegionName: mockRegion.Name})
		must.Nil(t, err)
