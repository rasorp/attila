...
func TestJobRegisterMethod_Create(t *testing.T) {
	...
	createResp1, errResp1 := testState.JobRegister().Method().Create(
		&store.JobRegisterMethodCreateReq{Method: mockMethod},
	)
	must.Nil(t, errResp1)
...
func TestJobRegisterMethod_Delete(t *testing.T) {
	...
	deleteResp1, errResp1 := testState.JobRegister().Method().Delete(
		&store.JobRegisterMethodDeleteReq{Name: mockMethod.Name},
	)
	must.Nil(t, errResp1)
	must.Eq(t, &store.JobRegisterMethodDeleteResp{}, deleteResp1)
...
func TestJobRegisterMethod_Get(t *testing.T) {
	...
	getResp1, err := testState.JobRegister().Method().Get(&store.JobRegisterMethodGetReq{Name: "method"})
	must.Error(t, err)
	must.Nil(t, getResp1)
	...
	getResp2, err := testState.JobRegister().Method().Get(&store.JobRegisterMethodGetReq{Name: mockMethod.Name})
	must.Nil(t, err)
